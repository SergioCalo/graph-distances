...
from ot_markov_distances import discounted_wl_infty, discounted_ours
from ot_markov_distances.ours.sinkhorn_value_iteration import sinkhorn_value_iteration
...
def compute_distance_matrix( set1: list[nx.Graph], set2: list[nx.Graph], 
    ...
    for (clip1_i, clip1), (clip2_i, clip2) in iterator:
        ...
        #distance
        st = time.time()
        distance, conv = discounted_wl_infty(markov1.to(device)[None, ...],
...
def compute_distance_matrix_ours(set1: list[nx.Graph], set2: list[nx.Graph],
                            attr_type=None,
                            device=torch.device("cpu"), q=.1, k=None, **wl_parameters, ):
    del k  # ignore k if it is in the params
    distance_matrix = torch.zeros(len(set1), len(set2), device=device)
    ...
    for (clip1_i, clip1), (clip2_i, clip2) in iterator:
        ...
        st = time.time()
        distance_matrix[clip1_i, clip2_i] = \
            discounted_ours(markov1.to(device)[None, ...],
                                markov2.to(device)[None, ...],
                                D[None, ...],
                                muX=mu1,
                                muY=mu2,
